...
> 课程链接：[Stanford CS336 Spring 2025 - Lecture 14](https://stanford-cs336.github.io/spring2025-lectures/?trace=var/traces/lecture_14.json)
...
### KenLM
**带 Kneser-Ney 平滑的 n-gram 模型** [Article](https://en.wikipedia.org/wiki/Kneser%E2%80%93Ney_smoothing)
*   KenLM：最初用于机器翻译的快速实现 [code](https://kheafield.com/code/kenlm/)
*   用于数据过滤的通用语言模型
*   极其简单 / 快速 - 只是计数和归一化
...
解决方案：使用 Kneser-Ney 平滑来处理未见过的 n-grams [Article](https://en.wikipedia.org/wiki/Kneser%E2%80%93Ney_smoothing)
...
### fastText
fastText 分类器 [Link](https://arxiv.org/pdf/1607.01759)
*   任务：文本分类（例如情感分类）
*   目标是训练一个用于文本分类的快速分类器
...
**实现：**
*   并行化，异步 SGD
*   学习率：从 [某个数字] 线性插值到 0 [Article](https://github.com/facebookresearch/fastText/blob/main/src/fasttext.cc#L653)
...
### DSIR
通过重要性重采样进行语言模型数据选择 (DSIR) [Link](https://arxiv.org/abs/2302.03169)
...
```
结果：在 [GLUE](https://gluebenchmark.com/) 基准测试上，DSIR 略优于启发式分类（fastText）
...
数据选择调查论文 [Link](https://arxiv.org/abs/2402.16827)
...
模型在多语言性方面有所不同：
*   English 仅占 BLOOM（训练不足）的 30%，英语性能受损 [Link](https://arxiv.org/pdf/2303.03915)
*   大多数前沿模型（GPT-4, Claude, Gemini, Llama, Qwen）都是高度多语言的（充分训练）

**fastText 语言识别** [Article](https://fasttext.cc/docs/en/language-identification.html)
*   现成的分类器
*   支持 176 种语言
*   在多语言网站上训练：Wikipedia, Tatoeba（翻译网站）和 SETimes（东南欧新闻）

**示例：Dolma 保留 p(English) >= 0.5 的页面** [Link](https://arxiv.org/abs/2402.00159)
...
基于单词特征训练线性分类器 [Article](https://spark.apache.org/docs/latest/ml-features#tokenizer)
根据分数随机保留文档：
```python
...
[HumanEval](https://huggingface.co/datasets/openai_humaneval) 结果：
...
**Dolma 中的毒性过滤** [Link](https://arxiv.org/abs/2402.00159)
数据集：Jigsaw 有毒评论数据集 (2018) [dataset](https://www.kaggle.com/datasets/julian3833/jigsaw-toxic-comment-classification-challenge)
*   项目目标：帮助人们在网上进行更好的讨论 [Article](https://www.kaggle.com/competitions/jigsaw-toxic-comment-classification-challenge/discussion/46064)
...
**两种类型的重复：**
*   精确重复（镜像站点，GitHub forks） [Gutenberg mirrors](https://www.gutenberg.org/MIRRORS.ALL)
*   近似重复：只有几个 tokens 不同的相同文本

**近似重复的例子：**
*   服务条款和许可证 [MIT license](https://opensource.org/license/mit)
*   公式化写作（复制/粘贴或从模板生成）
...
**去重训练数据使语言模型更好** [Link](https://arxiv.org/pdf/2107.06499)
*   训练更有效率（因为 tokens 更少）
*   避免死记硬背（可以减轻版权、隐私问题）
...
**效率与抗冲突性之间的权衡** [Article](https://softwareengineering.stackexchange.com/questions/49550/which-hashing-algorithm-is-best-for-uniqueness-and-speed)
...
**假阳性率分析** [Article](https://en.wikipedia.org/wiki/Bloom_filter)
假设哈希函数和项目独立。
*   m = bins 数量
*   k = 哈希函数数量
*   n = 插入的项目数量
...
结果假阳性率：f = 0.5^k
计算 (k)、内存 (m) 和假阳性率 (f) 之间的权衡 [Lecture notes](https://people.eecs.berkeley.edu/~daw/teaching/cs170-s03/Notes/lecture10.pdf)
...
**示例设置** [Link](https://arxiv.org/pdf/2107.06499)
*   n = 9000, b = 20, r = 450
*   阈值（发生相变的地方）≈ (1/b)^(1/r)
*   概率约为 1 - 1/e
