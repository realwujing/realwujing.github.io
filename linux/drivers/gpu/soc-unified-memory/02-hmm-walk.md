...
## HMM 在 SoC 统一内存中的角色：依赖但不依赖迁移

HMM 是 SoC 统一内存的**核心依赖**——`hmm_range_fault` 读 CPU 页表获取 PFN 数组是不可替代的。但 SoC 和 Discrete GPU 拿到 PFN 后做的事完全不同：

### Discrete GPU（H100）：HMM → 迁移 → GPU 内部页表

```
hmm_range_fault → PFN[]
  │
  ├─ PFN 是系统 DRAM → nouveau_pfns_map → NVIF 写 GPU 内部 MMU 页表
  └─ PFN 是 DEVICE_PRIVATE（数据在 VRAM）→ migrate_vma → DMA 搬运到 DRAM
     每个 PFN 都要经过 "这个页在 DRAM 还是 VRAM?" 的判断
```

### SoC（NVIDIA+MTK 笔记本 SoC）：HMM → 直接写 SMMU 页表

```
hmm_range_fault → PFN[]
  │
  └─ ★ PFN 全部是普通系统 PFN——物理 DDR 地址
     没有 DEVICE_PRIVATE 条目（GPU 和 CPU 共享同一块 LPDDR）
     → arm_smmu_map_pages → arm_lpae_map_pages → 写 SMMU io-pgtable
     → SMMU 页表条目 = PTE(pfn=PFN, prot=READ|WRITE)
     → GPU 通过 SMMU 再次访问该 IOVA → SMMU 页表命中 → 返回物理地址
```

**核心区别一句话**：

| | Discrete GPU（已讲 nvidia-svm 系列） | SoC UMA（本系列） |
|---|---|---|
| PFN 类型 | 可能是 DEVICE_PRIVATE（VRAM） | ★ 始终是普通系统 PFN |
| 需要迁移吗？ | 是（`migrate_vma`） | ★ 不需要——所有数据都在 DDR |
| 页表写入 | NVIF → GPU 内部 MMU 页表 | `arm_smmu_map_pages` → SMMU `io-pgtable` |
| DMA 拷贝？ | 可能（页迁移时） | ★ 永不发生 |

**Discrete GPU 的 DEVICE_PRIVATE 分支在 SoC 上完全不走**。`hmm_vma_handle_pte` 中的 DEVICE_PRIVATE 检查（`hmm.c:260-275`）永远不会匹配——因为没有页被标记为 DEVICE_PRIVATE。所有页都是 `pte_present && vm_normal_page` 的正常路径，返回的就是标准的 `pte_pfn(pte) | cpu_flags`。

### SoC 为什么还需要 HMM？

既然 CPU 和 GPU 共享同一块 DDR，直接让 GPU 通过 SMMU 访问物理地址不就行了？为什么还要走 HMM 读 CPU 页表？

**两件事**：

1. **CPU 页表可能有缺页**：`hmm_range_fault` 的 `hmm_vma_fault` 路径在 PTE 不存在时调用 `handle_mm_fault` 触发缺页处理——就像 CPU 自己访问该页一样。否则 SMMU 也查不到这个映射，GPU 访问直接 abort
2. **只读快照不需要锁**：`hmm_range_fault` 走 CPU 页表是只读操作（读 PTE，不写 PTE）——比 SMMU 硬件自己遍历页表更高效（走 CPU 的 TLB cache 路径），而且内核有 `mmap_lock` 保证页表一致性。SMMU 硬件遍历是异步的，需要 stall + IOPF 才能处理

**两者的协作**：

```
首次访问：IOPF handler → hmm_range_fault（读 CPU 页表）→ arm_smmu_map_pages（写 SMMU 页表）
                                     │                          │
                              利用 CPU TLB cache             后续访问全走 SMMU
                              缺页时自动 fault-in              不再经过 HMM
```

HMM 负责"发现 PFN"，SMMU 负责"翻译执行"。一次 HMM 调用后，PFN 被写进 SMMU 页表，后续所有 GPU 访问都直接走 SMMU 页表命中，不需要再经过 HMM。
