...
│  • 物理线缆上缓冲溢出被消除                       │
│  • 原因：信用协商是硬件逐跳的，收发双方预先协商     │
└────────────────────────────────────────────────┘
```

#### 链路层：硬 RDMA 真不丢 vs 软 RDMA 做不到

**硬 RDMA (InfiniBand)** — 逐跳硬件信用（credit）流控：

```
Host HCA ──[credits]──→ Switch ──[credits]──→ Target HCA
           "能收 32 个 VL15 buffer，发吧"
           "能收 16 个，发吧"
```

- **逐跳（per-hop）**：每根线缆两端独立协商 credits，不依赖端到端
- **按虚拟通道（VL）隔离**：每个 VL 有独立 credit 池，管理流量不会被数据流量饿死
- **硬件实现**：HCA/Switch ASIC 纳秒级响应，不经过 OS
- **发送方不发没有 buffer 的包** → 物理上没有缓冲溢出 → 链路层丢包不可能

这跟 TCP/IP 的根本区别在于设计哲学：

| | TCP/IP (Ethernet) | InfiniBand |
|---|---|---|
| **流控位置** | 端到端（TCP 滑动窗口） | 逐跳（每根线缆硬件 credit） |
| **拥塞信号** | 丢包 = 拥塞信号（后验） | credit 协商消除拥塞（前验） |
| **丢包态度** | 容忍丢包，丢了重传 | 设计上禁止丢包 |
| **中间交换机** | buffer 不够就 tail drop/RED | 每跳有 credit，不会溢出 |
| **以太网 PAUSE** | 全局停等，头阻严重，生产少用 | 不适用，IB 是自建网络 |

**软 RDMA (rxe)** — 没有链路层保护，走的是普通以太网 + 内核 IP/UDP 栈。链路层该丢照丢。

#### 传输层：无论软硬，RDMA 都有丢包和重传

即使硬 RDMA 链路层不丢包，传输层仍然会触发重传和 error：

```c
// include/rdma/ib_verbs.h:991-1016 — 传输层错误码
enum ib_wc_status {
    IB_WC_SUCCESS,
...
// drivers/infiniband/hw/mlx5/cq.c:325,329 — 网卡固件错误映射
...
| `min_rnr_timer` | RNR NAK 最小等待延迟 | 12 (≈1ms) |
| `timeout` | 本地 ACK 超时 `4.096 × 2^timeout` µs | 14 (≈67ms) |

这些参数在软硬 RDMA 上共用同一套代码：

```c
// rxe_qp.c:761-770 — rxe 和 mlx5 用同样的 ib_qp_attr 字段
qp->attr.retry_cnt = attr->retry_cnt;     // 重传上限
qp->comp.retry_cnt = attr->retry_cnt;
qp->attr.rnr_retry = attr->rnr_retry;     // RNR 重试上限
qp->comp.rnr_retry = attr->rnr_retry;
```

#### 传输层丢包的根本原因

这些场景下，即使链路层完好，传输层也会失败：

| 场景 | 机制 | 结果 |
|------|------|------|
| 接收端 QP recv queue 满了 | 网卡发 RNR NAK，发送方等 `min_rnr_timer` 后重试 | `rnr_retry` 耗尽 → `IB_WC_RNR_RETRY_EXC_ERR` |
| 接收端处理太慢没回 ACK | 发送方 `timeout` 超时触发重传 | `retry_cnt` 耗尽 → `IB_WC_RETRY_EXC_ERR` |
| 远端 MR key 无效 | 远端网卡校验失败 → 本地收到 NAK | `IB_WC_REM_ACCESS_ERR` |
| 光纤断/光模块坏 | 物理中断 | `retry_cnt` 耗尽 → QP error |
| RoCEv2 PFC 死锁/风暴 | 链路层无损失效 | 实际丢包 → ACK 超时 → 重传 |

#### 软 RDMA 的丢包恢复路径（代码佐证）

软 RDMA 没有链路层 credit，靠 **PSN 包序号 + 超时重传** 补回来：

**PSN 检测丢包：**

```c
// rxe_resp.c:78-93 — 根据 PSN diff 判断丢包、重复、乱序
if (diff > 0)  {                    // 期望 PSN 更大 → 前面有包丢了！
    qp->resp.sent_psn_nak = 1;      // 发 NAK 通知对方重传
    rxe_counter_inc(rxe, RXE_CNT_OUT_OF_SEQ_REQ);
    return RESPST_ERR_PSN_OUT_OF_SEQ;
} else if (diff < 0) {              // 收到的 PSN 更小 → 重复包
    rxe_counter_inc(rxe, RXE_CNT_DUP_REQ);
    return RESPST_DUPLICATE_REQUEST;
}
```

**RNR NAK Timer — 内核 timer 上的重传：**

```c
// rxe_req.c:101-115 — 对端 Recv Queue 满了，等 timer 再试
...
**重传逻辑 — req_retry() 重置 WQE 从头再发：**

```c
// rxe_req.c:37-94 — 收到 NAK 后重走整个 SQ
static void req_retry(struct rxe_qp *qp)
{
    qp->req.psn = qp->comp.psn;          // 回退 PSN
    for (...) {
        wqe->dma.resid = wqe->dma.length; // 重置 DMA 游标
        wqe->dma.cur_sge = 0;
        wqe->dma.sge_offset = 0;
        wqe->state = wqe_state_posted;    // 重新投递
    }
}
```
...
```
硬 RDMA (IB):
  链路层: credit 协商，绝对不丢 ──→ 传输层: 只处理 RNR/超时/违例
  丢包恢复: 不用管（硬件保证）

软 RDMA (rxe):
  链路层: 普通以太网，该丢就丢 ──→ 传输层: PSN 检测 + timer 超时重传
  丢包恢复: 靠内核 timer + workqueue 重传，和 TCP 逻辑类似

共同点:
  传输层都共用同一套 ib_qp_attr(retry_cnt/rnr_retry/timeout)
  都走 PSN 序号校验 → NAK → 重传的协议标准
```

#### 一句话总结

> **InfiniBand 链路层是无损的（credit-based 硬件流控），RDMA 传输层仍然有重传和超时机制。RoCEv2 依赖 PFC 尝试实现链路无损但不稳定。软 RDMA 完全没有链路层保护，用 PSN 序号 + timer 重传模拟可靠交付。无论是硬是软，QP 的 `retry_cnt`/`rnr_retry`/`timeout` 参数都是为了应对"丢"而存在的。**
