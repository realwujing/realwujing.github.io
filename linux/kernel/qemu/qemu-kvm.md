...
| **虚拟化类型** | **实现方式**       | **前端驱动名称** | **后端驱动名称**            | **前端驱动位置** | **后端驱动位置** | **数据包是否经过 QEMU 转发** | **控制信息**                                                                 | **控制信息是否经过 QEMU 转发** | **数据包传递机制**                     | **中断透传是否用到** | **中断透传的实现方式**                                                                 | **是否使用 eventfd** | **是否使用 ioeventfd** | **是否使用 vring** | **数据收发包机制**               | **数据收包链路**                                                                 | **数据发包链路**                                                                 | **是否使用 SR-IOV** | **是否使用 IOMMU** |
|----------------|--------------------|------------------|-----------------------------|------------------|------------------|-----------------------------|-----------------------------------------------------------------------------|-------------------------------|----------------------------------------|----------------------|--------------------------------------------------------------------------------------|----------------------|------------------------|--------------------|----------------------------------|----------------------------------------------------------------------------------|----------------------------------------------------------------------------------|--------------------|-------------------|
| **全虚拟化**   | **纯软件模拟**     | `virtio-net`     | QEMU 用户空间模拟            | 虚拟机内部        | 物理机用户空间    | 是                          | virtio 配置、队列状态、事件通知                                              | 是                            | 中断                                   | 否                   | 中断由 QEMU 模拟，虚拟机通过虚拟中断机制接收中断信号。                                | 否                   | 否                     | 是                  | 中断                             | 物理网络 → 宿主机内核网络栈 → QEMU → vring → 虚拟机前端驱动 → Guest OS           | Guest OS → 虚拟机前端驱动 → vring → QEMU → 宿主机内核网络栈 → 物理网络           | 否                 | 否                |
| **半虚拟化**   | **vhost-net**      | `virtio-net`     | `vhost-net`（内核驱动）      | 虚拟机内部        | 物理机内核空间    | 否                          | virtio 配置、队列状态、事件通知                                              | 是                            | eventfd（事件驱动）                    | 否                   | 使用 eventfd 机制替代中断，虚拟机通过事件驱动机制接收通知。                            | 是                   | 是                     | 是                  | eventfd + ioeventfd               | 物理网络 → 宿主机内核网络栈 → vhost-net → vring → 虚拟机前端驱动 → Guest OS       | Guest OS → 虚拟机前端驱动 → vring → vhost-net → 宿主机内核网络栈 → 物理网络       | 否                 | 是                |
| **半虚拟化**   | **vhost-user**     | `virtio-net`     | vhost-user（用户空间，通常由 OVS + DPDK 管理） | 虚拟机内部        | 物理机用户空间    | 否                          | virtio 配置、队列状态、事件通知                                              | 否                            | 共享内存 + eventfd                     | 否                   | 使用 eventfd 机制替代中断，虚拟机通过事件驱动机制接收通知。                            | 是                   | 是                     | 是                  | eventfd + ioeventfd               | 物理网络 → OVS + DPDK → vhost-user → vring → 虚拟机前端驱动 → Guest OS            | Guest OS → 虚拟机前端驱动 → vring → vhost-user → OVS + DPDK → 物理网络            | 否                 | 是                |
| **半虚拟化**   | **vDPA**           | `virtio-net`     | `vdpa-net`（网络设备）<br>`vdpa-blk`（块设备） | 虚拟机内部        | 物理机硬件设备    | 否                          | virtio 配置、队列状态、事件通知                                              | 否                            | 硬件加速（绕过软件栈）                 | 是                   | 硬件设备的中断直接传递给虚拟机，绕过宿主机的软件栈。                                   | 否                   | 否                     | 是                  | 硬件中断                         | 物理网络 → 硬件设备 → vring → 虚拟机前端驱动 → Guest OS                           | Guest OS → 虚拟机前端驱动 → vring → 硬件设备 → 物理网络                           | 是                 | 是                |
| **VFIO**       | **VFIO**           | 硬件设备原生驱动（如 `ixgbe`、`mlx5_core`） | `vfio-pci`（PCI 设备）<br>`vfio-platform`（平台设备） | 虚拟机内部        | 物理机硬件设备    | 否                          | 硬件设备配置、队列状态、事件通知                                             | 否                            | 硬件中断（直接访问硬件）               | 是                   | 硬件设备的中断直接传递给虚拟机，绕过宿主机的软件栈。                                   | 否                   | 否                     | 否                  | 硬件中断                         | 物理网络 → 硬件设备 → 虚拟机原生驱动 → Guest OS                                   | Guest OS → 虚拟机原生驱动 → 硬件设备 → 物理网络                                   | 是                 | 是                |
