...
内核发现内存不足时通过中断触发了内存回收，根据crash ps来看，用户态进程鼻祖1号进程被杀了，内核态线程鼻祖2号进程也被杀，应该是启动了OOM。
...
- 水位线百分比的推荐值通常在 50 到 90 之间。较低的值意味着更早触发OOM Killer，以保持可用内存的较高水平。较高的值则更容易允许内存分配，但可能增加系统发生OOM的风险。

替换 `<value>` 为您希望设置的水位线百分比值，例如 50 表示50%。这里将OOM调节器设置为经典模式（`vm.overcommit_memory = 0`），意味着系统根据实际内存使用情况触发OOM Killer。

```bash
sudo sysctl -p
```

这将重新加载配置并应用新的OOM调节器设置。
