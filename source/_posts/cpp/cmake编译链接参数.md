---
date: 2023/04/21 15:49:27
updated: 2023/04/21 15:49:27
---

# cmake编译链接参数

## cmake编译链接参数

- [cmake编译选项](https://blog.51cto.com/u_15064646/3462077)
- [GCC编译选项参数](https://zhuanlan.zhihu.com/p/513148985)
- [添加-O0 -g，生成gdb的调试信息和不进行代码优化](https://blog.51cto.com/u_15064646/3462077)
- [【使用CMake组织C++工程】2：调试包不优化](https://elloop.github.io/tools/2016-04-10/learning-cmake-2-commands)
- [CMake解决c++11的phread库问题：undefined reference to `pthread_create’](https://blog.csdn.net/sandalphon4869/article/details/102503224)
- [cmake编译缺失pthread_create等thread库解决方案](https://www.jianshu.com/p/10bf5d27dcfc)
- [gcc -lpthread和gcc -pthread的区别](http://blog.chinaunix.net/uid-69906223-id-5817313.html)
- [CMake: link options](https://www.5axxw.com/questions/content/ratnvg)
- [CMake：如何在CMakeLists.txt中设置LDFLAGS？](https://www.dovov.com/cmakecmakelists-txtldflags.html)

- [CMAKE 中　add_definitions的用法．](https://blog.csdn.net/dongjideyu/article/details/79267683)
- [cmake:设置编译选项的讲究(add_compile_options和CMAKE_CXX_FLAGS的区别)](https://blog.csdn.net/10km/article/details/51731959)
- [ubuntu16.04cmake、gcc/g++版本更换](https://blog.csdn.net/fb_941219/article/details/88638503)

## Dynamic code analysis

- [cmake 内存泄露检查asan](https://www.cnblogs.com/gladiatorplus/p/14437059.html)
- [动态代码分析 Clion](https://www.jetbrains.com/help/clion/profiling-tools.html)
- [libasan使用和遇到的问题](https://blog.csdn.net/register_k/article/details/120319834)
